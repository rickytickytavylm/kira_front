// Кира — веб. Флоу: лендинг → iOS-онбординг → чат. Два режима (эксперт/школа),
// профиль собеседника, стриминг ответа (SSE), подписка (заглушка), персист.
(function () {
  "use strict";
  ...
  const TRANSLATIONS = {
    ru: {
      "meta.title": "Кира — ИИ-помощник Доктора Шурова В.А.",
      "meta.description": "Кира — анонимный ИИ-психолог-аддиктолог, обученный доктором Василием Шуровым. Поможет разобраться с зависимостью, созависимостью, тревогой и депрессией. Анонимно, 24/7.",
      "nav.doctor": "Доктор", "nav.benefits": "Возможности", "nav.pricing": "Тарифы", "nav.start": "Начать",
      "hero.eyebrow": "ИИ-помощник доктора Шурова В.А.", "hero.title": "Здравствуйте,<br>я&nbsp;Кира",
      "hero.sub": "Анонимный ИИ-психолог-аддиктолог, обученный доктором Василием Шуровым. Помогу честно и без осуждения разобраться с зависимостью, созависимостью, тревогой и депрессией — на связи 24/7.",
      "hero.cta.start": "Начать разговор", "hero.cta.pricing": "Тарифы&nbsp;· 990&nbsp;₽",
      ...
      "benefits.1": "Анонимный разговор 24/7", "benefits.2": "Начать без звонка, без стыда и страха",
      "benefits.3": "Бережный разбор вашей ситуации", "benefits.4": "Помощь при зависимости и созависимости",
      "benefits.5": "Поддержка для родственников", "benefits.6": "Ответы о тревоге, депрессии и психиатрии",
      "benefits.7": "Подскажу подходящий формат помощи", "benefits.8": "Переход к помощи клиник Василия Шурова",
      "benefits.quote": "Иногда самый сложный шаг — не лечение, а первый честный разговор. Кира создана для того, чтобы этот разговор стал легче.",
      "benefits.cta": "Попробовать бесплатно — первый день",
      ...
      "tabbar.home": "Главная", "tabbar.doctor": "Доктор", "tabbar.pricing": "Тарифы", "tabbar.chat": "Чат",
      ...
      "greet.prefix": "Здравствуйте", "greet.name_prefix": "Здравствуйте,", "greet.intro": "Я Кира, ИИ-психолог-аддиктолог школы доктора Шурова.",
  ...
  // ─── Хранилище ───
  const LS = { device: "kira_device_id", profile: "kira_profile", history: "kira_history", mode: "kira_mode" };
  ...
  let mode = load(LS.mode, "expert");

  // Мульти-чаты (как в ChatGPT)
  let chats = load("kira_chats", null);
  let currentId = load("kira_current", null);
  if (!Array.isArray(chats)) {
    ...
    save("kira_chats", chats); save("kira_current", currentId);
  }
  const saveChats = () => { save("kira_chats", chats.slice(0, 40)); save("kira_current", currentId); };
  ...
  $("#benefitsCta").addEventListener("click", openSub);
  $("#heroPricing").addEventListener("click", openSub);
  $("#navPricing").addEventListener("click", openSub);
  ...
  // ═══════════ ПОДПИСКА (заглушка) ═══════════
  function openSub() { openSheet(subModal); }
  $("#subClose").addEventListener("click", () => closeSheet(subModal));
  subModal.addEventListener("click", (e) => { if (e.target === subModal) closeSheet(subModal); });
  $("#subStart").addEventListener("click", () => {
    // TODO: здесь подключить реальную оплату (ЮKassa и т.п.). Пока — пробный доступ.
    save("kira_trial_started", Date.now());
    closeSheet(subModal);
  ...
  function switchChat(id) {
    ...
    save(LS.profile, profile); save("kira_current", id);
  ...
  function setMode(m) {
    ...
    const c = curChat(); if (c) { c.mode = m; saveChats(); }
  ...
  async function submit(prefill) {
    ...
    try {
      const resp = await fetch(`${BACKEND}/api/chat`, {
        ...
        body: JSON.stringify({ messages: msgs, profile, mode, deviceId: DEVICE, lang: currentLang }),
      });
